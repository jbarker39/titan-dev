...
class DBobj:
		...
		def     __init__(self,dbname):
				self.hostname = dbname[0]
				self.username = dbname[1]
				self.passwd = dbname[2]
				self.usedb = dbname[3]

				try:
						#print "hostname: %s username: %s password: %s db: %s" % (self.hostname,self.username,self.passwd,self.usedb)
						self.conn = MySQLdb.connect (self.hostname,self.username,self.passwd,self.usedb)
						...
						if t:
								for i in t:
										...
										if asset.cust_id:
												...
												customer_name = "TBD"
										print "%s,%s,%s,%s,%s,%s,%s" % (asset.fqdn,mfg,mn,rn,sn,asset.warranty_date,customer_name)
				...
				if table == 'assets':
						...
						for l in f0:
								...
								if asset.model_name != "VM":
										...
										if not mid:
												use_mid = 0
												print "model name: %s not found" % asset.model_name
										else:
												use_mid = mid[0]
										...
										if not rid:
												use_rid = 0
												print "%s rack name: %s not found" % (asset.fqdn,asset.rack_name)
										else:
												use_rid = rid[0]
						...
						for l in f0:
								...
								rid = self.cursor.fetchone()
								if not rid:
										print "rack name: %s not found" % (asset.rack_name)
								else:
						...
						for l in f0:
								...
								if not mid:
										use_mid = 0
										#print l
										print "model_name: %s not found" % (asset.model_name)
								else:
										use_mid = mid[0]
								self.cursor.execute("""SELECT rack_id from racks where rack_name like %s""",(asset.rack_name))
								rid = self.cursor.fetchone()
								if not rid:
										use_rid = 0
										print l
										print "rack_name: %s not found" % asset.rack_name
								else:
										use_rid = rid[0]
								...
								if arecord:
										...
										tfqdn = arecord[1]
										tsn = arecord[2]
										print "LOOKUP by asset_tag:: asset_id: %s fqdn: %s asset_tag: %s serial_number: %s starting_u: %s" % (taid,tfqdn,asset.asset_tag,tsn,asset.starting_u)
										#self.cursor.execute("""UPDATE assets set starting_u = %s where asset_id = %s""",(asset.starting_u,taid))
										#self.conn.commit()
										if not tfqdn:
												print "ERROR: %s is NULL should be %s -- UPDATED" % (tfqdn,asset.fqdn)
										...
										if arecord:
												...
												tfqdn = arecord[1]
												tsn = arecord[2]
												print "LOOKUP by serial_number:: asset_id: %s fqdn: %s asset_tag: %s serial_number: %s starting_u: %s" % (taid,tfqdn,asset.asset_tag,tsn,asset.starting_u)
												#self.cursor.execute("""UPDATE assets set starting_u = %s where asset_id = %s""",(asset.starting_u,taid))
												#self.conn.commit()
												if not tfqdn:
														print "ERROR: %s is NULL should be %s -- UPDATED" % (tfqdn,asset.fqdn)
												...
												tfqdn = "NA"
												tsn = "NA"
												print "asset_tag: %s not found" % (asset.asset_tag)
						...
						for l in f0:
								...
								if not rid:
										use_rid = 0
										print "rack name: %s not found" % (rack_info.rack_name)
								else:
										use_rid = rid[0]
						...
						for l in f0:
								...
								mid = self.cursor.fetchone()
								if mid:
										print f.fqdn,f.model_name,f.sn,mid
						...
						for line in f0:
								...
								if r:
										#print r
										print "%s,%s,%s,%s,%s" % (f.serial_number,r[1],r[3],r[2],r[0])
								#else:
		...
		def update_slc(self,rec):
				...
				if rid:
						...
						mid = self.cursor.fetchone()
						if not mid:
								print "model name: %s not found" % (rec.model_name)
								return False
						...
						print "Rack Name: %s not found" % (rec.rack_name)
						return False
				return True
		...
		def update_su_newqwest(self,asset_tag,su):
			...
			if self.cursor.execute("""UPDATE assets set starting_u = %s where asset_tag like %s""",(su,asset_tag)):
					...
					print "asset_tag: %s not found" % (asset_tag)
...
#assets.asset_id,fqdn,serial_number,asset_tag,rack_id,model_id,assets.cust_id,warranty_date,starting_u
		...
		def     get_by_racks(self):
				self.cursor.execute("""SELECT starting_u,rack_name,model_name,fqdn,serial_number,customer_name from \
									...
									customers.cust_id = assets.cust_id order by _rack_name""")
				c = self.cursor.fetchall()
				return c
		...
		def	update_from_ARMupdate(self,linein):
			...
			if model_id:
				...
				self.conn.commit()
				print "model_id %s model_name %s asset_tag %s" % (model_id[0],linein[0],linein[1].strip())
			else: print "model_name: %s not found" % (linein[0])
			#
		def decom_report(self):
			self.cursor.execute("""SELECT fqdn,asset_tag,serial_number,model_name,rack_name,decom from assets,racks,models where \
			assets.rack_id=racks.rack_id and assets.model_id=models.model_id and decom not like %s and decom not like %s""",("in service","available"))
			return self.cursor.fetchall()
		...
		def	qham2_update_rackable(self,sn,at): #get information from qahm2
			...
			if r:
				...
				self.conn.commit()
				print "asset_id: %s updated with asset_tag: %s and decom: %s" % (r[0],at,"2012")
		...
		def	qham2_newslc_part2(self,line): #update the information in newslc
			...
			r = self.cursor.fetchone()
			if r:
				print "asset_d: %s line %s %s %s %s %s %s %s" % (r[0],line[0],line[1],line[2],line[3], line[4],line[5],line[6])
			else:
				print "asset_tag %s not found" % (line[0])
		...
		def update_rack_id(self,at,rack_name):
			...
			if rack_id:
				...
				self.conn.commit()
				print "Updated rack_id %s for rack_name %s at asset_tag %s" % (rack_id[0],rack_name,at)
		def update_asset_id(self,oaid,aid):
				self.cursor.execute ("""UPDATE assets set asset_id = %s where asset_id = %s""",(oaid,aid))
				self.conn.commit()  
		
		
		def load_ALL_assets_fromDB(self):
				self.cursor.execute("""SELECT asset_id,fqdn,serial_number,asset_tag,warranty_date,rack_id,model_id,starting_u from assets where 1""")
				c = self.cursor.fetchall()
				return c
